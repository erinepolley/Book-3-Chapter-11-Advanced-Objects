...
const salesBox = document.querySelector("#container")
let domOutput = ""
// for (let sale in salesByWeek) {
//     domOutput += Object.entries(sale.vehicle)
// }

// console.log(domOutput)

salesByWeek.forEach(sale => {
    domOutput += `<hr><h2>${sale.sales_agent.first_name} ${sale.sales_agent.last_name}</h2>
`
    for (const vehicleProperty of Object.entries(sale.vehicle)) {
        domOutput += `<p>${vehicleProperty[0]}: ${vehicleProperty[1]}</p>`
    }
